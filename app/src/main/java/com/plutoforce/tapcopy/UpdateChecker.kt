...
/**
 * Checks GitHub for a newer TapCopy build and installs it over the top.
 *
 * The release publishes a small version.json next to the APK; we compare its
 * versionCode with the installed one. Everything here is plain networking to
 * GitHub only — no screen data ever leaves the device.
 */
object UpdateChecker {

    private const val VERSION_URL =
        "https://github.com/legacybuilder0011/app-releases/releases/download/latest/version.json"
    ...
    /** Fetches the latest published version info, or null on any failure. Blocking. */
    ...
    /** Downloads the update APK into external files/updates. Blocking. */
    fun downloadApk(context: Context, apkUrl: String): File? {
        return try {
            val dir = File(context.getExternalFilesDir(null), "updates").apply { mkdirs() }
            val file = File(dir, "TapCopy-update.apk")
